...
- [ ] #14 [stack: solo] Wire the project-approval form's data (internal notes, priority, tech stack, budget override, dates) through to persistence — currently silently dropped.
  ...
  - There's currently no UI anywhere under `src/app` that displays `projects.internalNotes` after approval — add a display for it to whatever admin/dev-facing project view is most appropriate at implementation time (e.g. project detail page, once one exists — see task #1's note that no authed "existing project detail" page exists yet).
...
- [ ] #2 [stack: invoicing] Add "features" as advanced request options (enum), collapsed by default in the request form. Per docs/INVOICE_BILLING_EPIC.md Phase 1: also expand the ProjectFeature enum with the new values listed there and create featurePricing.ts mapping each feature to { label, defaultPrice, description }.
  ...
  - Post-implementation taxonomy refinement: after this task landed, the ProjectFeature list and the separate `ProjectRequirements` object were reviewed together against real invoicing scenarios (e.g. e-commerce inventory integration had no feature, ECommerce/MobileApp defaults were missing PaymentProcessing/Auth) and revised. `ProjectRequirements` was retired entirely — everything billable moved into `ProjectFeature`, which is now derived as a per-`ProjectType` default and fully editable. The enum itself changed: dropped `ThirdPartyIntegrations` and `ProjectManagement`, renamed `Testing`→`QaLaunchTesting` (scoped to manual/launch QA only, not automated coverage) and `Consultation`→`TechnicalAdvisory` (to stop colliding with `ProjectType.Consultation`), and added `CustomDesign`, `EcommercePlatformIntegration`, `ContentCreation`, `MaintenanceRetainer`. Code no longer matches this task's original 17-value list above — see `src/graphql/schema/project.ts` and `src/libs/featurePricing.ts` for the current set.
